<template>
    ...
    >
        ...
        <button v-if="isRead" class="btn btn-link mr-2" @click.stop="markAsUnread" :dusk="`mark-as-unread-${notification.id}`">
            <i class="far fa-circle"></i>
            <span class="position-absolute bg-dark text-white ml-2 py-1 px-2 rounded">Marcar como NO leída</span>
        </button>
        <button v-else class="btn btn-link mr-2" @click.stop="markAsRead" :dusk="`mark-as-read-${notification.id}`">
